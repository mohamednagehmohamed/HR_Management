...
namespace Persons
{
    public partial class myjoin : Form
    {
        ...
        void getalldate()
        {
            ...
            {
                con.Open();
                string query = "select askarynum as'الرقم العسكري',name as'الاسم',numid as'الرقم القومي',age as'العمر',startdate as 'تاريخ التسريح',enddate as'تاريخ الانتهاء' from myjoined";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    con.Open();
                    string query = "insert into myjoined values('" + int.Parse(txtnumaskary.Text) + "',N'" + txtname.Text + "','" + int.Parse(txtid.Text) + "','" + int.Parse(txtage.Text) + "',N'" + txtstartdate.Text + "','" + txtenddate.Text + "')";
                    SqlCommand cmd = new SqlCommand(query, con);
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("تمت اضافة بيانات الانثي");
                    con.Close();
                    getalldate();
        ...
        private void myjoin_Load(object sender, EventArgs e)
        {
            getalldate();
            lblrows.Text = (dataGridView1.Rows.Count - 1).ToString();
        }
        ...
        private void txtsearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                string query = "select askarynum as'الرقم العسكري',name as'الاسم',numid as'الرقم القومي',age as'العمر',startdate as 'تاريخ التسريح',enddate as'تاريخ الانتهاء' from myjoined where name like N'%" + txtsearch.Text + "%'";
                SqlDataAdapter sda = new SqlDataAdapter(query, con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                dataGridView1.DataSource = dt;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    con.Open();
                    string query = "update myjoined set askarynum='" + int.Parse(txtnumaskary.Text) + "',name=N'" + txtname.Text + "',numid='" + int.Parse(txtid.Text) + "',age='" + int.Parse(txtage.Text) + "',startdate=N'" + txtstartdate.Text + "',enddate=N'" + txtenddate.Text + "'where askarynum='" + int.Parse(txtnumaskary.Text) + "'";
                    SqlCommand cmd = new SqlCommand(query, con);
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("تم تحديث بيانات الانثي", "مكتب النظم", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    con.Close();
                    getalldate();
                }
                catch (Exception ex)
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    con.Open();
                    string query = "delete from myjoined where askarynum='" + int.Parse(txtnumaskary.Text) + "'";
                    SqlCommand cmd = new SqlCommand(query, con);
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("تم حذف بيانات الانثي ", "مكتب النظم", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    con.Close();
                    getalldate();
                    lblrows.Text = (dataGridView1.Rows.Count - 1).ToString();
                }
